    ...
    <AppShell>
      <PageWrapper>
        ...
        <section className="mb-8 grid gap-4 sm:grid-cols-3">
          <article className="flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">
              <BookOpen className="h-6 w-6" />
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Available Tests</p>
              <h3 className="text-xl font-bold text-slate-800 mt-1">{stats.available}</h3>
            </div>
          </article>
          ...
          <article className="flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            ...
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Tests Attempted</p>
              <h3 className="text-xl font-bold text-slate-800 mt-1">{stats.attempted}</h3>
            </div>
          </article>
          ...
          <article className="flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            ...
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Average Performance</p>
              <h3 className="text-xl font-bold text-slate-800 mt-1">{stats.accuracy}%</h3>
            </div>
          </article>
        </section>
        ...
        {activeTab === "exams" && (
          <>
            {/* Filter Controls */}
            <div className="mb-6 grid gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_240px]">
              ...
              <select
                ...
                className="h-11 rounded-md border border-slate-300 bg-white px-4 text-sm text-slate-700 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              >
              ...
              <div className="grid gap-6 md:grid-cols-2">
                {filteredTests.map((test) => {
                  ...
                  return (
                    <article
                      key={test.id}
                      className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md duration-200"
                    >
                      <div>
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-2 line-clamp-1">{test.name}</h3>
                        ...
                        <div className="mb-5 grid grid-cols-3 gap-2 text-xs text-slate-500 border-y border-slate-100 py-3 mt-4">
                          <div className="flex flex-col items-center">
                            <span className="font-semibold text-slate-700">{test.total_questions}</span>
                            <span>Questions</span>
                          </div>
                          ...
                          <div className="flex flex-col items-center">
                            <span className="font-semibold text-slate-700">{test.total_marks}</span>
                            <span>Marks</span>
                          </div>
                        </div>
                      </div>

                      <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50 border-dashed gap-4">
                        <div>
                          {attempt ? (
                        ...
                        <div className="flex gap-2">
                          {attempt ? (
                            test.results_shared ? (
                              ...
                              <Button disabled variant="secondary" className="h-9 text-xs px-3.5 text-slate-400 cursor-not-allowed">
                                Results Pending
                              </Button>
                            )
                          ) : (
                            isSlotActive && test.questions && test.questions.length > 0 ? (
                              ...
                              <Button disabled className="h-9 text-xs px-4 bg-slate-100 text-slate-400 border-transparent">
          ...
          <div className="space-y-4">
            ...
            {attempts.filter(a => a.user_id === user?.userId).length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-300 bg-white py-16 text-center text-slate-500">
                ...
                <p className="mt-1 text-sm text-slate-400">You haven't completed any exams yet. Go attempt active tests!</p>
              </div>
            ) : (
              <div className="grid gap-6 md:grid-cols-2">
                {attempts
                  ...
                  .map((attempt) => {
                    ...
                    return (
                      <article
                        key={attempt.id}
                        className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md duration-200"
                      >
                        <div>
                          <div className="mb-3 flex flex-wrap items-center gap-2">
                            <Badge tone="blue">{subjectsName}</Badge>
                          ...
                          <h3 className="text-lg font-bold text-slate-800 mb-2">{attempt.test_name || test?.name || "Exam Attempt"}</h3>
                          <div className="text-xs text-slate-500 space-y-1 mb-4">
                            <div>Submitted on: <span className="font-semibold text-slate-700">{formattedDate}</span></div>
                            <div>Time Spent: <span className="font-semibold text-slate-700">{Math.floor(attempt.time_spent / 60)}m {attempt.time_spent % 60}s</span></div>
                          </div>
                          <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-2">
                            <div>
                              <span className="text-xs text-slate-400 block uppercase font-bold">Marks Scored</span>
                            ...
                            {test?.results_shared ? (
                              ...
                              <Button disabled variant="secondary" className="text-slate-400 text-xs h-9 px-4 cursor-not-allowed">
                                Results Pending
                              </Button>
                            )}
                          </div>
